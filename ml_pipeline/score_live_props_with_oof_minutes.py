...
MINUTES_MODEL_PATH = "ml_pipeline/models/minutes_model_context_final.pkl"
POINTS_MODEL_PATH = "ml_pipeline/models/points_model_with_oof_minutes.pkl"
CALIBRATION_PATH = "ml_pipeline/models/points_model_calibration_with_oof_minutes.pkl"
...
OUTPUT_PATH = "ml_pipeline/data/live_scored_props_with_oof_minutes.csv"
PLAYER_ALIASES = {
...
def main():
    ...
    with open(POINTS_MODEL_PATH, "rb") as file:
    ...
    points_feature_columns = list(points_model.feature_name_)
    ...
    for prop in props.itertuples(index=False):
        ...
        reconstructed = {}
        for column in set(minutes_feature_columns) | set(points_feature_columns):
            if column in state.index:
                reconstructed[column] = state[column]
    ...
    X_minutes = reconstructed_df[minutes_feature_columns].copy()
    reconstructed_df["predicted_minutes_oof"] = minutes_model.predict(X_minutes)
    reconstructed_df["predicted_minus_recent_minutes_oof"] = (
        reconstructed_df["predicted_minutes_oof"] - reconstructed_df["min_last_5"]
    )

    X_points = reconstructed_df[points_feature_columns].copy()
    reconstructed_df["predicted_points"] = points_model.predict(X_points)
    reconstructed_df["predicted_minutes"] = reconstructed_df["predicted_minutes_oof"]
    ...
    summary = output["best_edge"].describe(percentiles=[0.1, 0.25, 0.5, 0.75, 0.9])
    print("edge distribution summary:")
